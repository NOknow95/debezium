...
public final class RangeSplitUtil {
    ...
    private static final Pattern SIMPLE_UUID_PATTERN = Pattern.compile("(\\w{8})(\\w{4})(\\w{4})(\\w{4})(\\w{12})");

    public static void main(String[] args) {
        String[] split = doUuidStringSplit("b2ac5fc6-cc1f-11ec-b296-0242ac110003", "f823f63a-cc1e-11ec-b296-0242ac110003", 3);
        System.out.println(Arrays.asList(split));

        long[] longs = doLongSplit(1000, 1009, 3);
        System.out.println(Arrays.toString(longs));
    }

    public static String[] doAsciiStringSplit(String left, String right, int expectSliceNumber) {
        //处理UUID的情况
        boolean isUuid = UUID_PATTERN.matcher(left).matches() && UUID_PATTERN.matcher(right).matches();
        if (isUuid) {
            return doUuidStringSplit(left, right, expectSliceNumber);
        }
        int radix = 128;
    ...
    public static String[] doUuidStringSplit(String left, String right, int expectSliceNumber) {
        ...
        for (int i = 1, len = tempResult.length - 1; i < len; i++) {
            result[i] = bigIntegerToUuidString(tempResult[i]);
        }

        return result;
    }

    private static String bigIntegerToUuidString(BigInteger bigInteger) {
        String hex = bigInteger.toString(16);
        Matcher matcher = SIMPLE_UUID_PATTERN.matcher(hex);
        if (matcher.matches()) {
            List<String> list = new ArrayList<>(matcher.groupCount() - 1);
